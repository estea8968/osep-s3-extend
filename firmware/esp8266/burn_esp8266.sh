...
. /opt/osep-s3-extend/firmware/esp8266/config
...
if [ $u_choice -eq 1 ];then 
	esptool.py --port $1 --baud 460800 write_flash --flash_size=detect -fm dio 0 /opt/osep-s3-extend/firmware/esp8266/esp8266-20191220-v1.12.bin
	ampy --port $1 put /opt/osep-s3-extend/firmware/esp8266/main.py
	ampy --port $1 put /opt/osep-s3-extend/firmware/esp8266/esp_8266_min.py 
